...
if (($progressCheck -eq "Incomplete") -or ($progressCheck -eq "Failed")) {
    try {
        ...
        if ($progressCheck -eq "Failed") {
            ...
            Get-AzsGalleryItem | Where-Object { $_.Name -like "*AzureStackPOC*" } | Remove-AzsGalleryItem -ErrorAction SilentlyContinue
        }
        ...
        if (-not ($azsStorageAccount)) {
            ...
            $azsStorageAccount = New-AzStorageAccount -Name $azsImagesStorageAccountName -Location $azsLocation -ResourceGroupName $azsImagesRGName -SkuName Standard_LRS -ErrorAction Stop
        }
        Write-Host "Setting the storage context"
        Set-AzCurrentStorageAccount -Name $azsImagesStorageAccountName -ResourceGroupName $azsImagesRGName | Out-Null
